...
public class Serwer extends Thread {
    ...
    private String[][] gameboardP1 = null;
    private String[][] gameboardP1enemy = null;
    private String[][] gameboardP2 = null;
    private String[][] gameboardP2enemy = null;
    ...
    public void game(String inP, PrintWriter outputP, PrintWriter outputPEnemy) {
        ...
        switch (in[1]) {
            case "PS":
                placingShips(outputP, in);
                break;
            case "SH":
                if (in[0].equals("P1")) {
                    if (gameLogic.isShotPossible(in[2] + " " + in[3], gameboardP2, gameboardP1enemy)) {
                        gameLogic.makeAShot(in[2] + " " + in[3], gameboardP2, gameboardP1enemy);
                        if (gameLogic.isGameWon(gameboardP2)) {
                            outputP.println("Win");
                ...
                if (in[0].equals("P2")) {
                    if (gameLogic.isShotPossible(in[2] + " " + in[3], gameboardP1, gameboardP2enemy)) {
                        gameLogic.makeAShot(in[2] + " " + in[3], gameboardP1, gameboardP2enemy);
                        if (gameLogic.isGameWon(gameboardP1)) {
                            outputP.println("Win");
                            outputPEnemy.println("Lost");
                            try {
                                socketP1.close();
    ...
    private void placingShips(PrintWriter outputP, String[] in) {
        if (in[0].equals("P1")) {
